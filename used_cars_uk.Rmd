...
```{r}
# Carga de los datos
used_cars <- read.csv("/Users/margom/OneDrive - KI.SE/Mac/Documents/Gomez_Marta/UOC-Data Science/Tipologia y Ciclo de Vida de los Datos/Practica 2 (PEC4)/aprox100KUsedCars.csv", stringsAsFactors = TRUE)
...
En el segundo análisis haremos un caso práctico de compra-venta por parte de un vendedor en busca de "chollos".
...
# Test de Lilliefors
library("nortest")
lillie.test(x = top_10_cars$mileage)
lillie.test(x = top_10_cars$mpg)
lillie.test(x = top_10_cars$tax)
lillie.test(x = top_10_cars$price)
lillie.test(x = top_10_cars$year)
lillie.test(x = top_10_cars$engineSize)
```


## Análisis Estadístico

Para ayudar a nuestro cliente a tomar una decisión vamos a aplicar una serie de test estadísticos.
* 1. Comparación precio medio por modelo.
...
price <- top_10_cars %>%
  group_by(model) %>%
  summarise_at(vars(price), list(precio_medio = mean, error = sd))
...
```
Del anterior análisis seleccionamos tres modelos (A Class, Focus, Golf) que dadas sus características creemos que le pueden interesar al comprador y estudiamos:
...
En este caso estudiamos dicho impacto en cada uno de los modelos por separado. 
...
Finalmente estudiamos la relación del tipo de combustible en el precio y kilometraje según el modelo.
...
Este análisis no aporta mucha información, sin embargo los gráficos nos permiten concluir que 
